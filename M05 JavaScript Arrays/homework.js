...
function incrementarPorUno(array) {
   ...
   // Tu código:
   const arregloIncrementado = array.map(numero => numero + 1);
...
function dePalabrasAFrase(palabras) {
   ...
   // Tu código:
   var palabrasConEspacios = palabras.join(' ');
   return palabrasConEspacios;
}
...
function todosIguales(array) {
   ...
   // Tu código:
   var elementosIguales = array.every(item =>{
      return item === array[0];
      })
...
function breakStatement(num) {
   ...
   for (let i = 0; i < 10; i++) {
      num += 2;
      arrayPorDos.push(num);
      if(num === i){
         break;
      }
   }
   if (arrayPorDos.length < 10) {
